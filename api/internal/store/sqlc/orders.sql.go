...
const buscarPedidoPorReferencia = `-- name: BuscarPedidoPorReferencia :one
SELECT id, branch_id, external_id
FROM orders
WHERE source = $1 AND external_id = $2
`

type BuscarPedidoPorReferenciaParams struct {
	Source     *Procedencia `json:"source"`
	ExternalID *string      `json:"external_id"`
}

type BuscarPedidoPorReferenciaRow struct {
	ID         uuid.UUID   `json:"id"`
	BranchID   pgtype.UUID `json:"branch_id"`
	ExternalID *string     `json:"external_id"`
}
...
func (q *Queries) BuscarPedidoPorReferencia(ctx context.Context, arg BuscarPedidoPorReferenciaParams) (BuscarPedidoPorReferenciaRow, error) {
	row := q.db.QueryRow(ctx, buscarPedidoPorReferencia, arg.Source, arg.ExternalID)
	var i BuscarPedidoPorReferenciaRow
